<template>
    <div class='hundred dx-withdlist'>
        <div v-infinite-scroll='load' infinite-scroll-disabled='disabled' style="overflow:auto">
            ...
            <!-- </ul> -->
            <div class="listbox"  v-for='(item, index) in teamList' :key='index'>
              <div class="list-row">
                <span>申请提现：</span>
                <span class="col">&yen;{{item.trxAmt}}</span>
              </div>
              <div class="list-row">
                <span>申请时间：</span>
                <span class="col">{{$moment(item.createTime.time).format('YYYY-MM-DD HH:mm:ss')}}</span>
              </div>
              <div class="list-row">
                <span>申请状态：</span>
                <span :class="item.status == '10C' ? 'suc' : (item.status == '10D' ? 'err' : 'jin')">{{type[item.status]}}</span>
              </div>
            </div>
        </div>
